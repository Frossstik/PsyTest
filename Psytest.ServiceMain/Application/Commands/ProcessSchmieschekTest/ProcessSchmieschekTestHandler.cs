...
{
    public class ProcessSchmieschekTestHandler : IRequestHandler<ProcessSchmieschekTestCommand, TestResult>
    {
        private readonly MainDbContext _db;
        private readonly SchmieschekTestProcessor _processor;

        public ProcessSchmieschekTestHandler(MainDbContext db, IEnumerable<ITestProcessor> processors)
        {
            _db = db;
            _processor = processors.OfType<SchmieschekTestProcessor>().First();
        }

        public async Task<TestResult> Handle(ProcessSchmieschekTestCommand request, CancellationToken cancellationToken)
        {
