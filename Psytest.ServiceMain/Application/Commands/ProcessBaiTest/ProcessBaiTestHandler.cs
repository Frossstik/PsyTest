...
{
    public class ProcessBaiTestHandler : IRequestHandler<ProcessBaiTestCommand, TestResult>
    {
        private readonly MainDbContext _db;
        private readonly BaiTestProcessor _processor;

        public ProcessBaiTestHandler(MainDbContext db, IEnumerable<ITestProcessor> processors)
        {
            _db = db;
            _processor = processors.OfType<BaiTestProcessor>().First(); ;
        }
        public async Task<TestResult> Handle(ProcessBaiTestCommand request, CancellationToken cancellationToken)
        {
